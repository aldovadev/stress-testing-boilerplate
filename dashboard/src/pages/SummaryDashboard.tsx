  ...
  if (!summary || !summary.metrics) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-100">Test Summary</h2>
          <p className="text-sm text-gray-500 mt-1">Post-run aggregated results</p>
        </div>
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-12 text-center">
          <Clock className="w-10 h-10 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-400">No test results available yet.</p>
          <p className="text-gray-600 text-sm mt-1">Run a test to see the summary here.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {meta && (
          <div className="flex items-center gap-4 text-xs text-gray-500">
            <span className="px-2 py-1 rounded bg-gray-800 text-gray-400 uppercase font-medium">
              {meta.phase}
            </span>
      ...
      {checks.length > 0 && (
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-300">Checks</h3>
              ...
              {checks.map((c, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                    ...
                    <td className="px-4 py-2.5 text-right text-emerald-400 font-mono text-xs">{c.passes}</td>
                    <td className="px-4 py-2.5 text-right text-red-400 font-mono text-xs">{c.fails}</td>
                    <td className="px-4 py-2.5 text-center">
                      {rate === 100 ? (
                        <CheckCircle className="w-4 h-4 text-emerald-400 inline-block" />
                      ) : (
                        <span className="text-xs font-mono text-amber-400">{rate.toFixed(1)}%</span>
                      )}
                    </td>
                  </tr>
                );
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider">Data Sent</p>
            <p className="text-lg font-bold text-gray-100">
              {formatBytes(dataSent?.values.count ?? 0)}
            </p>
            <p className="text-xs text-gray-500">{(dataSent?.values.rate ?? 0).toFixed(0)} bytes/s</p>
          </div>
        </div>
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4 flex items-center gap-4">
          <ArrowDownToLine className="w-8 h-8 text-purple-400" />
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider">Data Received</p>
            <p className="text-lg font-bold text-gray-100">
              {formatBytes(dataRecv?.values.count ?? 0)}
            </p>
            <p className="text-xs text-gray-500">{(dataRecv?.values.rate ?? 0).toFixed(0)} bytes/s</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/50 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-800">
          <h3 className="text-sm font-medium text-gray-300">All Metrics</h3>
        </div>
            ...
            <tbody>
              {Object.entries(metrics).sort(([a], [b]) => a.localeCompare(b)).map(([name, metric]) => (
                <tr key={name} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-300">{name}</td>
